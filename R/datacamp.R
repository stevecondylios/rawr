...
#' @name datacamp
#'
#' @usage datacamp(url, method, delimiter)
#'
...
datacamp <- function(url, method = 1, delimiter = delimiter) {

  page_html <- xml2::read_html(url)

  if(method == 1) {

    if(missing(delimiter)) { delimiter = "\n\n" }

    page_html %>% gsub("\\{r\\}", "r", .) %>% read_html %>%
    html_nodes(".lang-r") %>% html_text %>% paste0(collapse=delimiter)
    ...
    page_html %>% html_nodes(xpath="//div[@data-datacamp-exercise=true()]") %>% html_text %>%
      ...
      sapply(function(x) { x$sample }) %>% unname %>% paste0(collapse=delimiter)
